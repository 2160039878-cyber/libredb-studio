...
| Materialize | `postgres` | Partial | Materialize 26.40.0 | **Re-probed after #38680: the object browser now lists tables and columns** (previously nothing but the editor worked). The schema query retries around four gaps in turn - the reserved `MATERIALIZED` keyword, the missing `pg_total_relation_size()` builtin, `json_agg()`/`json_build_object()` (only the `jsonb_` forms exist), and `information_schema.constraint_column_usage` - recovering real data instead of failing outright. Foreign keys stay **absent rather than wrong** and indexes come back empty: `constraint_column_usage` is the only one of the three constraint views Materialize lacks and the only one naming the table a key points at, so the fallback empties the FK part of the query instead of dropping the tables and columns that were readable all along. The `'[]'::json` casts in the same statements are left alone - the cast was measured working on a live instance even though Materialize documents no `json` type. **Materialized views are listed beside tables**, with their columns: Materialize reports them as `table_type = 'MATERIALIZED VIEW'` and they are its central object, so listing only `BASE TABLE` hid the product (no other engine emits that type - measured on all six). **All seven monitoring tabs render without an engine error**: the table, index and tablespace reads answer no rows on a missing `pg_` object rather than failing their panel. Row counts read 0 for everything, which is a claim rather than a reading - `pg_class.reltuples` answers -1 there and `getSchema()` clamps a negative to 0, the same clamp stock PostgreSQL hits on a freshly loaded table. The monitoring dashboard now loads too, every statistic reading unavailable rather than the whole page erroring: no pg statistics catalog, no size functions. |
