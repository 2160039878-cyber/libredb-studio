...
| Materialize | `postgres` | Partial | Materialize 26.40.0 | **Re-probed after #38680: the object browser now lists tables and columns** (previously nothing but the editor worked). The schema query retries around three gaps in turn - the reserved `MATERIALIZED` keyword, the missing `pg_total_relation_size()` builtin, and `json_agg()`/`json_build_object()` (only the `jsonb_` forms exist) - recovering real data instead of failing outright. Foreign keys and indexes stay unavailable: Materialize has no `information_schema.constraint_column_usage`, and the two-phase schema load reports that failure without blocking the table list. The monitoring dashboard now loads too, every statistic reading unavailable rather than the whole page erroring: no pg statistics catalog, no size functions. |
