...
export const WIRE_COMPATIBLE_ENGINES: readonly WireCompatibleEngine[] = [
  ...
  },
  {
    ...
    caveats: [
      ...
      "The monitoring dashboard loads with every statistic marked unavailable rather than erroring the whole page: Materialize has no pg statistics catalog and no size functions. All seven tabs render, none of them printing an engine error: the table, index and tablespace reads answer no rows on a missing pg_ object the way the session and slow-query reads already did.",
      ...
      "Row counts read 0 for every object, which is a claim rather than a reading: Materialize answers -1 from pg_class.reltuples - PostgreSQL's never-analyzed sentinel - for tables, views and materialized views alike, and getSchema() clamps a negative to 0. Stock PostgreSQL hits the same clamp on a freshly loaded table until autovacuum runs, so this is not Materialize-specific.",
    ],
  },
  {
    name: "RisingWave",
